...
class WorkoutNotes2ViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate {
    ...
    @IBAction func inputButton(_ sender: Any) {
        ...
        if exerView.text != ""
        {
            ref?.child("users").child(Auth.auth().currentUser!.uid).child("notes2").childByAutoId().setValue(exerView.text)
            exerView.text = ""
        }
        else
            if exerView.text == "" {
    ...
    override func viewDidLoad() {
        ...
        handle = ref?.child("users").child(Auth.auth().currentUser!.uid).child("notes2").observe(.childAdded, with: { (snapshot) in
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            ...
            DispatchQueue.main.asyncAfter(deadline: when, execute: {
                self.ref?.child("users").child(Auth.auth().currentUser!.uid).child("notes2").child(self.keyArray[indexPath.row]).removeValue()
    ...
    func GetAllKeys() {
        ref?.child("users").child(Auth.auth().currentUser!.uid).child("notes2").observeSingleEvent(of: .value, with: { (snapshot) in
