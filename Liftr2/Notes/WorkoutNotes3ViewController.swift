...
class WorkoutNotes3ViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    @IBAction func completeWorkout(_ sender: Any) {
        ...
        let work1 = UIAlertAction(title: "Completed \(navigationItem.title!) and Other Workouts", style: .default, handler: { action in self.performSegue(withIdentifier: "comp3", sender: self)})
    ...
    @IBAction func inputButton(_ sender: Any) {
        ...
        if exerView.text != "" {
            ...
            ref?.child("user").child(Auth.auth().currentUser!.uid).child("workout notes").child("notes 3").childByAutoId().setValue(dict)
            exerView.text = ""
