//
//  DailyChallenge2ViewController.swift
//  Liftr2
//
//  Created by Connor Berry on 12/03/2018.
...
class Challange2ViewController: UIViewController {
    ...
    func giveRewardsAndSetScoresToZero() {
        ...
        ref.child("user/\(user)/bronze reward").setValue(newBronze)
        ref.child("user/\(user)/silver reward").setValue(newSilver) // put the new scores into the database
        ref.child("user/\(user)/gold reward").setValue(newGold)
