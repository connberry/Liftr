...
class WaistViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate {
    ...
    @IBAction func inputButton(_ sender: Any) {
        ...
        if mesView.text != "" {
            ...
            mesView.text = ""
            let alertController = UIAlertController(title: "Nice One!", message: "Your waist has been stored. If you want to amend, just enter a new weight and it will be overwritten for today. 💪", preferredStyle: .alert)
    ...
    override func viewDidLoad() {
        ...
        // Adds notes child
        handle = ref?.child("user").child(Auth.auth().currentUser!.uid).child("measurements").child("waist").observe(.childAdded, with: { (snapshot) in
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            ...
            DispatchQueue.main.asyncAfter(deadline: when, execute: {
                self.ref?.child("user").child(Auth.auth().currentUser!.uid).child("measurements").child("waist").child(self.keyArray[indexPath.row]).removeValue()
    ...
    func GetAllKeys() {
        ref?.child("user").child(Auth.auth().currentUser!.uid).child("measurements").child("waist").observeSingleEvent(of: .value, with: { (snapshot) in
    ...
    // How button pressed
    @IBAction func How(_ sender: Any) { UIApplication.shared.open(URL(string: "https://www.wikihow.com/Measure-Your-Waist")! as URL, options: [:], completionHandler: nil)
    }
}
