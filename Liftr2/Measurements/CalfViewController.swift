...
class CalfViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate {
    ...
    @IBAction func inputButton(_ sender: Any) {
        ...
        if mesView.text != "" {
            ...
            ref?.child("user").child(Auth.auth().currentUser!.uid).child("measurements").child("calf").child("\(getDate())").setValue(dict)
            mesView.text = ""
            let banner = NotificationBanner(title: "Success, Calves have been saved 🤙", style: .success)
            banner.show()
        }
        else
