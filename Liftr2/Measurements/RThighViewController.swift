...
class RThighViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate {
    ...
    @IBAction func inputButton(_ sender: Any) {
        ...
        if mesView.text != "" {
            ref?.child("user").child(Auth.auth().currentUser!.uid).child("measurements").child("right thigh").childByAutoId().setValue(mesView.text)
            mesView.text = ""
        }
        else
    ...
    override func viewDidLoad() {
        ...
        // Adds notes child
        handle = ref?.child("user").child(Auth.auth().currentUser!.uid).child("measurements").child("right thigh").observe(.childAdded, with: { (snapshot) in
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            ...
            DispatchQueue.main.asyncAfter(deadline: when, execute: {
                self.ref?.child("user").child(Auth.auth().currentUser!.uid).child("measurements").child("right thigh").child(self.keyArray[indexPath.row]).removeValue()
    ...
    func GetAllKeys() {
        ref?.child("user").child(Auth.auth().currentUser!.uid).child("measurements").child("right thigh").observeSingleEvent(of: .value, with: { (snapshot) in
