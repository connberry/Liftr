...
class HeightViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate {
    ...
    @IBAction func inputButton(_ sender: Any) {
        ...
        if mesView.text != "" {
            ...
            ref?.child("user").child(Auth.auth().currentUser!.uid).child("measurements").child("height").child("\(getDate())").setValue(dict)
            mesView.text = ""
            let banner = NotificationBanner(title: "Success, Height has been saved 🤙", style: .success)
            banner.show()
        }
        else
